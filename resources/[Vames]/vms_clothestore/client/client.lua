...
RegisterNUICallback("openBill", function(data)
    ...
    for k, v in pairs(currentValue) do
        if v.newValue ~= v.value then
            if (Config.PricesForIdsSpecificStores[storeIsIn] and Config.PricesForIdsSpecificStores[storeIsIn][k] and Config.PricesForIdsSpecificStores[storeIsIn][k][tostring(v.newValue)]) then
                receipt[k] = Config.PricesForIdsSpecificStores[storeIsIn][k][tostring(v.newValue)]
            elseif (Config.PricesForComponentSpecificStores[storeIsIn] and Config.PricesForComponentSpecificStores[storeIsIn][k]) then
                receipt[k] = Config.PricesForComponentSpecificStores[storeIsIn][k]
            elseif (Config.PricesForIds[k] and Config.PricesForIds[k][tostring(v.newValue)]) then
                receipt[k] = Config.PricesForIds[k][tostring(v.newValue)]
            elseif (Config.PricesForComponent[k]) then
